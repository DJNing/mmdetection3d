...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty, is_vod=False):
    gt_datas_list = []
    dt_datas_list = []
    total_dc_num = []
    ...
    for i in range(len(gt_annos)):
        if is_vod:
            rets = clean_data_vod(gt_annos[i], dt_annos[i], current_class, difficulty)
        else:
...
def eval_class_vod(gt_annos,
               ...
               compute_aos=False,
               num_parts=200):
    """VOD eval. support 2d/bev/3d/aos eval. support 0.5:0.05:0.95 coco AP.
    ...
    for m, current_class in enumerate(current_classes):
        for idx_l, difficulty in enumerate(difficultys):
            rets = _prepare_data(gt_annos, dt_annos, current_class, difficulty, is_vod=True)
            (gt_datas_list, dt_datas_list, ignored_gts, ignored_dets,
             dontcares, total_dc_num, total_num_valid_gt) = rets
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                thresholdss = []
...
def vod_eval(gt_annos,
               dt_annos,
               current_classes,
               eval_types=['bbox', 'bev', '3d']):
    """VOD evaluation.
        remove truncated value in _prepare_data(clear_data)
    Args:
        gt_annos (list[dict]): Contain gt information of each sample.
        dt_annos (list[dict]): Contain detected information of each sample.
        current_classes (list[str]): Classes to evaluation.
    ...
    mAP11_bbox, mAP11_bev, mAP11_3d, mAP11_aos, mAP40_bbox, mAP40_bev, \
        mAP40_3d, mAP40_aos = do_eval_vod(gt_annos, dt_annos,
                                      current_classes, min_overlaps,
                                      eval_types)
