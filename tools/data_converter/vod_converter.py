...
from .kitti_data_utils import WaymoInfoGatherer, get_vod_image_info
...
def create_kitti_info_file(data_path,
    ...
    print('\n' + '='*30)
    print('Generating info for train set')
    if save_path is None:
        save_path = Path(data_path)
    else:
        save_path = Path(save_path)
    kitti_infos_train = get_vod_image_info(
        data_path,
        training=True,
        velodyne=True,
        calib=True,
    ...
    _calculate_num_points_in_gt(data_path, kitti_infos_train, relative_path, num_features=num_feature)
    filename = save_path / f'{pkl_prefix}_infos_train.pkl'
    print(f'Kitti info train file is saved to {filename.absolute()}')
    print('\n' + '='*30)
    print('Generating info for val set')
    mmcv.dump(kitti_infos_train, filename.absolute())
    kitti_infos_val = get_vod_image_info(
        data_path,
        training=True,
        velodyne=True,
        calib=True,
        ...
        relative_path=relative_path)
    _calculate_num_points_in_gt(data_path, kitti_infos_val, relative_path, num_features=num_feature)
    ...
    print('\n' + '='*30)
    print('Generating info for test set')
    kitti_infos_test = get_vod_image_info(
        data_path,
        training=False,
        label_info=False,
        velodyne=True,
